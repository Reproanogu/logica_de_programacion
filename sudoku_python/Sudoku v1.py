...
#Validación
##Validar fila
 def validar_fila(tablero, fila, numero_seleccionado):
    for j in range(9):
        if tablero[fila][j] == numero_seleccionado:
            return False
    return True

##Validar columna
 def validar_columna(tablero, columna, numero_seleccionado):
    for i in range(9):
        if tablero[i][columna] == numero_seleccionado:
            return False
    return True

##Validar cuadrante
 def validar_cuadrante(tablero, fila, columna, numero_seleccionado):
    inicio_fila = (fila//3) * 3
    inicio_columna = (columna//3) * 3
    for i in range(inicio_fila, inicio_fila + 3):
        for j in range(inicio_columna, inicio_columna + 3):
            if tablero[i][j] == numero_seleccionado: 
                return False
    return True 

##Validar según soluciones
